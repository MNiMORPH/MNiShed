...
.. figure:: _static/crow_wing_seasonal.png
   :width: 80%
   :align: center

   Crow Wing seasonal mass balance (2001–2010, a calibrated run): modeled
   discharge split into fast/event, slow/baseflow, and lake-outlet sources versus
   observed Q. The lake carries a large, steady share in every season; the fast
   response peaks in the spring (MAM) freshet. Generated by
   ``docs/figures/plot_crow_wing_seasonal.py``.
