...
.. figure:: _static/recession_bn.png
   :width: 70%
   :align: center

   The recession "cloud": :math:`-\mathrm{d}Q/\mathrm{d}t` against :math:`Q` in
   log-log space, with the fitted power law
   :math:`-\mathrm{d}Q/\mathrm{d}t = a\,Q^{n}`. The exponent :math:`n` (here
   :math:`\approx 1.6`, between linear storage and a faster late-time recession)
   maps to a reservoir recession exponent prior. Generated by
   ``docs/figures/plot_recession.py``.
