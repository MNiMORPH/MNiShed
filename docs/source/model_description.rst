...
.. seealso::
   ...
   The :doc:`example_crow_wing` worked example uses a lake series element,
   together with two parallel land zones and growing-degree-day phenology, on a
   real lake-rich basin.
...
.. figure:: _static/frozen_ground.png
   :width: 85%
   :align: center

   Effect of the frozen-ground module on the Cannon example: blocking
   infiltration while the ground is frozen routes more meltwater straight to the
   stream, sharpening the spring freshet relative to the same model with the
   module off. Generated by ``docs/figures/plot_frozen_ground.py``.
...
.. figure:: _static/pdm_saturation.png
   :width: 65%
   :align: center

   The PDM saturated fraction :math:`f_\text{sat} = 1 - e^{-H/H_0}` against
   storage for several characteristic depths :math:`H_0`. A smaller :math:`H_0`
   saturates — and sheds saturation-excess runoff — at lower storage. Generated
   by ``docs/figures/plot_pdm.py``.
...
**Two tile-drain representations:**
  ...
  *storage-state dependent*: it contributes nothing below the threshold
  ...
  .. figure:: _static/tile_multipath.png
     :width: 75%
     :align: center

     Multipath drainage is storage-state dependent: a reservoir receding from
     high storage drains fast while above the drain depth (the threshold-activated
     path), then reverts to the slow matrix recession once storage falls below it.
     Generated by ``docs/figures/plot_tile.py``.
