...
.. figure:: _static/seasonal_mass_balance.png
   :width: 75%
   :align: center

   Modeled discharge split by source (fast/event, slow/baseflow, lake outlet) by
   season, against observed Q. A season carried by the slow store with Q over
   observed points to baseflow not receding; a flat per-source contribution
   points to an unresponsive reservoir. Generated by
   ``docs/figures/plot_seasonal_mass_balance.py``.
...
.. figure:: _static/identifiability.png
   :width: 100%
   :align: center

   Identifiability around a Cannon fit. Left: the curvature eigenspectrum
   (stiffness per eigen-direction). Middle: the eigenvector loadings — read down a
   sloppy column to name the degenerate combination. Right: a 2-D ridge over the
   soil recession timescale and the exfiltration fraction, whose diagonal valley
   is the degeneracy the data cannot separate. Generated by
   ``docs/figures/plot_identifiability.py``.
