...
def test_recharge_overflow_hmax():
    res = Reservoir(t_recession=10.0, Hmax=50.0, H0=40.0)
    res.recharge(20.0)
...
def test_discharge_linear_exponential_decay():
    ...
    tau = 10.0
    H0 = 100.0
    res = Reservoir(t_recession=tau, H0=H0)
...
def test_discharge_water_balance():
    ...
    res = Reservoir(t_recession=10.0, f_to_discharge=0.6, H0=80.0)
    H_before = res.Hwater
    res.discharge(dt=1.0)
...
def test_discharge_partitioning():
    ...
    f = 0.3
    res = Reservoir(t_recession=10.0, f_to_discharge=f, H0=100.0)
    res.discharge(dt=1.0)
...
def test_discharge_empty_reservoir_produces_nothing():
    res = Reservoir(t_recession=10.0, H0=0.0)
    res.discharge(dt=1.0)
    assert res.H_exfiltrated == pytest.approx(0.0)
    assert res.Hwater == pytest.approx(0.0)


def test_mrt_linear_equals_t_recession():
    """MRT of a linear reservoir equals t_recession regardless of Q_ref."""
    res = Reservoir(t_recession=42.0)
...
def test_invalid_t_recession_raises():
    with pytest.raises(ValueError):
        Reservoir(t_recession=0.0)
...
def test_invalid_f_to_discharge_raises():
    with pytest.raises(ValueError):
        Reservoir(t_recession=10.0, f_to_discharge=1.5)
...
def test_nonlinear_discharge_exceeds_linear():
    ...
    H0 = 4.0
    tau = 10.0
    res_linear = Reservoir(t_recession=tau, H0=H0)
    res_linear.discharge(dt=1.0)

    res_nonlinear = Reservoir(t_recession=tau, H0=H0)
...
def test_nonlinear_discharge_water_balance():
    ...
    res = Reservoir(t_recession=10.0, f_to_discharge=0.6, H0=4.0)
...
def test_tile_drain_increases_discharge():
    ...
    res_plain = Reservoir(t_recession=10.0, f_to_discharge=0.5, H0=100.0)
    res_plain.discharge(dt=1.0)

    res_tile = Reservoir(t_recession=10.0, f_to_discharge=0.5,
